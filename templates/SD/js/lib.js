...
$(function(){
	$(".orating_res").each(function(){
		...
		if(rateval>39) $(this).parent().parent().addClass("btn-success").removeClass("btn-secondary");
		else if(rateval>0) $(this).parent().parent().addClass("btn-success").removeClass("btn-secondary");
		else if(rateval<-39) $(this).parent().parent().addClass("btn-danger").removeClass("btn-secondary");
		else if(rateval<0) $(this).parent().parent().addClass("btn-danger").removeClass("btn-secondary");
	});
	$(".ocomm_res").each(function(){
		rateval = parseInt($(this).html());
		if(rateval>0) $(this).parent().parent().addClass("btn-success").removeClass("btn-secondary");
		else if(rateval<0){
			$(this).parent().parent().addClass("btn-danger").removeClass("btn-secondary");
			if(rateval<-4){
				var id = $(this).parent().parent().attr('id').split("-");
				hidecomm[id[1]] = $("#comment-id-"+id[1]).html();
				$("#comment-id-"+id[1]).html('<div class="comm_hide">Этот комментарий получил слишком много отрицательных голосов. <a href="#" onclick="showcomm('+id[1]+');return false;">Показать?</a></div>');
			}
		}
	});
})
